...
const PurchaseButton: FC<PurchaseButtonProps> = ({ slice }) => {
  ...
  useGSAP(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const normalizedX = Math.max(0, Math.min(mouseX / buttonWidth, 1));
    ...
    >
      ...
      >
        ...
        <button
          ...
          disabled={isPressed}
          className={clsx(
            "group relative w-full overflow-hidden rounded-full border-8 border-gray-900 bg-linear-to-r/oklch from-sky-300 to-sky-600 px-6 py-6 transition-all duration-300 ease-out focus:ring-24 focus:ring-sky-500/50 focus:outline-none md:border-12 md:px-20 md:py-16",
